...
- **FAD YAML frontmatter schema specified in FRAMEWORK.md §6.3:** Inlined canonical 8-field schema matching FOUNDING-ARCHITECTURE.template.md.
- **Agent guidance and domain fallback in GENERATOR.md:** Example agent prompt updated to record decisions exclusively in research/DECISIONS.md. Step 1 updated with custom archetype fallback for novel non-software domains.
- **Typography and attribution integrity:** Replaced corrupted UTF-8 replacement characters and PowerShell scripting artifacts in FRAMEWORK.md with proper unicode symbols (—, –, ≥, ±). Corrected Premortem protocol header attribution in PHASE-0-GATE.template.md to Klein (2007) and Mitchell et al. (1989). Reconciled project pipeline count across ROADMAP.md and README.md.
